import { GRAMMAR_BANK } from '../data/questions'
import type {
  ActivePuzzle,
  BoxCount,
  BoxCell,
...
function makeKindsByCount(boxCount: BoxCount): BoxKind[] {
  if (boxCount === 12) {
    return [
      'bomb', 'bomb',
      'bonus',
      'double_points',
      'sentence_fix', 'sentence_fix', 'sentence_fix', 'sentence_fix',
      'sentence_fix', 'sentence_fix', 'sentence_fix', 'sentence_fix',
    ]
  }
  if (boxCount === 16) {
  ...
  return [
    ...
    'sentence_fix', 'sentence_fix', 'sentence_fix', 'sentence_fix', 'sentence_fix',
    'sentence_fix', 'sentence_fix', 'sentence_fix', 'sentence_fix', 'sentence_fix',
    'sentence_fix', 'sentence_fix', 'sentence_fix', 'sentence_fix', 'sentence_fix',
  ]
}

function makeBoxes(boxCount: BoxCount): BoxCell[] {
  const kinds = makeKindsByCount(boxCount)
...
export function createNewSession(config: SetupConfig): GameSession {
  ...
  const roundCount = config.roundCount ?? config.boxCount
  return {
    teams,
    ...
    turnIndex: 0,
    turnNumber: 1,
    boxes: makeBoxes(config.boxCount),
    activePuzzle: null,
    gameOver: false,
    winnerTeamIds: [],
...
export function buildQuestionForBox(kind: BoxKind, difficulty: Difficulty, usedQuestionIds: string[]): PuzzleQuestion | undefined {
  if (kind === 'word_puzzle' || kind === 'sentence_fix' || kind === 'vocab_match' || kind === 'spelling_challenge') {
...
function getDifficultyBasePoints(difficulty: Difficulty) {
  return difficulty === 'intermediate' ? 15 : 10
}

function getFastBonusPoints(difficulty: Difficulty) {
  return difficulty === 'intermediate' ? 7 : 5
}
...
function awardAnswer(
  ...
  const nextTeams = teams.map((team, idx) => {
    ...
    usedDouble = team.nextDouble
    if (!correct) {
      earnedPoints = 0
      return { ...team, nextDouble: false }
    }
    const base = getDifficultyBasePoints(difficulty)
    const bonus = fast ? getFastBonusPoints(difficulty) : 0
    earnedPoints = (base + bonus) * multiplier
    return { ...team, points: team.points + earnedPoints, nextDouble: false }
  })
  return { teams: nextTeams, earnedPoints, usedDouble }
}
...
export function submitPuzzleAnswer(session: GameSession, rawAnswer: string): GameSession {
  ...
  const expectedAnswer =
    q.type === 'sentence_fix' ? q.corrected : q.type === 'word_puzzle' ? q.word : q.answer
  ...
  const award = awardAnswer(session.teams, session.turnIndex, session.difficulty, correct, fast)
  const boxes = markBoxResolved(session.boxes, puzzle.boxId)
  const activeTeam = session.teams[session.turnIndex]
  const logMsg = correct
    ? `${activeTeam.name} TRUE ✅ (+${award.earnedPoints}${fast ? ' fast bonus' : ''}${award.usedDouble ? ' x2' : ''})`
    : `${activeTeam.name} FALSE ❌ (0 ball)`

  const next = addLog({ ...session, teams: award.teams, boxes, activePuzzle: { ...puzzle, resolved: true } }, logMsg)
...
export function forceMarkPuzzle(session: GameSession, correct: boolean): GameSession {
  ...
  const award = awardAnswer(session.teams, session.turnIndex, session.difficulty, correct, false)
  const boxes = markBoxResolved(session.boxes, puzzle.boxId)
  const activeTeam = session.teams[session.turnIndex]
  const next = addLog(
    { ...session, teams: award.teams, boxes, activePuzzle: null },
    `${activeTeam.name}: teacher override (${correct ? `TRUE +${award.earnedPoints}` : 'FALSE 0'})`,
  )
