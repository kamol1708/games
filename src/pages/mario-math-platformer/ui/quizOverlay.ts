...
export function showQuizOverlay(options: QuizOptions): Promise<boolean> {
  ...
  let onSubmit: ((event: Event) => void) | null = null
  let onButtonClick: ((event: MouseEvent) => void) | null = null
  let onEnterKey: ((event: KeyboardEvent) => void) | null = null

  const cleanup = () => {
    if (intervalId != null) window.clearInterval(intervalId)
    if (onSubmit) els.form.removeEventListener('submit', onSubmit)
    if (onButtonClick) els.submit.removeEventListener('click', onButtonClick)
    if (onEnterKey) document.removeEventListener('keydown', onEnterKey)
